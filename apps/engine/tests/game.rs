...
use game::{
    CombatSystem, GamePosition, GameWorldContract, GameWorldContractClient, Health, MovementSystem,
};

/// Helper function to create a test environment with a deployed contract
fn setup_test_contract<'a>() -> (Env, GameWorldContractClient<'a>) {
    let env = Env::default();
...
fn setup_initialized_contract<'a>() -> (Env, GameWorldContractClient<'a>) {
...
mod contract_initialization_tests {
    ...
    fn test_contract_initialization() {
        ...
        assert_eq!(
            client.entity_count(),
            0,
            "Initial entity count should be zero"
        );
        assert_eq!(
            client.dead_entity_count(),
            0,
            "Initial dead entity count should be zero"
        );
    }
    ...
    fn test_multiple_initialization() {
        ...
        assert_eq!(
            ...
            "Multiple initialization should not change entity count"
        );
    }
}
...
mod entity_management_tests {
    ...
    fn test_entity_creation() {
        ...
        assert_eq!(
            client.entity_count(),
            1,
            "Entity count should be 1 after spawning one entity"
        );
        ...
        assert_eq!(
            client.entity_count(),
            2,
            "Entity count should be 2 after spawning two entities"
        );
    ...
    fn test_entity_position_retrieval() {
        ...
        assert!(
            position.is_some(),
            "Position should exist for spawned entity"
        );
    ...
    fn test_entity_despawning() {
        ...
        assert_eq!(
            client.entity_count(),
            1,
            "Should have 1 entity after spawning"
        );
        ...
        assert_eq!(
            client.entity_count(),
            0,
            "Should have 0 entities after despawning"
        );
        ...
        assert!(
            position.is_none(),
            "Position should not exist after despawning"
        );
...
mod movement_system_tests {
    ...
    fn test_movement_system_negative_boundary() {
        ...
        assert!(
            move_result,
            "Movement should succeed even with negative result"
        );
    ...
    fn test_movement_system_large_coordinates() {
        ...
        assert!(
            move_result,
            "Movement with large coordinates should succeed"
        );
        ...
        assert_eq!(
            position.0,
            large_x + 1000,
            "Large X coordinate movement should work"
        );
        assert_eq!(
            position.1,
            large_y - 1000,
            "Large Y coordinate movement should work"
        );
    }
...
mod combat_system_tests {
    ...
    fn test_combat_system_basic_attack() {
        ...
        assert!(
            position.is_some(),
            "Entity should still exist after non-fatal attack"
        );
    }
    ...
    fn test_combat_system_multiple_attacks() {
        ...
        for i in 1..=5 {
            ...
            let health = client.get_entity_health(&entity_id).unwrap();
            assert_eq!(
                health.0, expected_health as u32,
                "Health after attack {} should be {}",
                i, expected_health
            );
        }
        ...
        assert!(
            client.get_entity_position(&entity_id).is_some(),
    ...
    fn test_combat_system_health_underflow_protection() {
        ...
        assert!(
            client.get_entity_position(&entity_id).is_none(),
...
mod error_handling_tests {
    ...
    fn test_operations_on_nonexistent_entities() {
        ...
        assert!(
            position.is_none(),
            "Position retrieval on non-existent entity should return None"
        );
        ...
        assert!(
            health.is_none(),
            "Health retrieval on non-existent entity should return None"
        );
        ...
        assert!(
            !despawn_result,
            "Despawning non-existent entity should fail"
        );
    }
...
mod world_state_tests {
    ...
    fn test_empty_world_behavior() {
        ...
        assert_eq!(
            client.dead_entity_count(),
            0,
            "Empty world should have 0 dead entities"
        );
    ...
    fn test_world_state_consistency() {
        ...
        assert_eq!(
            client.entity_count(),
            2,
            "Should have 2 living entities after death"
        );
        assert_eq!(client.dead_entity_count(), 1, "Should have 1 dead entity");
        ...
        assert_eq!(
            client.entity_count(),
            1,
            "Should have 1 entity after despawn"
        );
    ...
    fn test_entity_id_uniqueness() {
        ...
        // Initial spawn should be sequential starting at 0
        for (index, &entity_id) in entity_ids.iter().enumerate() {
            assert_eq!(
                entity_id, index as u32,
                "Entity ID should be sequential at creation"
            );
        }
        ...
        // Spawn new entities – IDs may reuse freed slots
        let new_id1 = client.spawn_entity(&100, &200);
        let new_id2 = client.spawn_entity(&300, &400);

        // ✅ Check that new IDs are unique among *current* live entities
        let mut all_ids = Vec::new();
        for i in 0..client.entity_count() {
            if let Some(_) = client.get_entity_position(&i) {
                all_ids.push(i);
            }
        }
        ...
        sorted.sort();
        sorted.dedup();
        assert_eq!(sorted.len(), all_ids.len(), "All live IDs should be unique");

        // ✅ Ensure the two new entities actually exist
        assert!(all_ids.contains(&new_id1));
        assert!(all_ids.contains(&new_id2));
    }
}
...
mod integration_tests {
    ...
    fn test_full_gameplay_scenario() {
        ...
        let enemy2_id = client.spawn_entity(&0u32, &5u32); // Enemy 2 (will clamp to 5, 5)

        assert_eq!(
            client.entity_count(),
            3,
            "Should have 3 entities in the game"
        );
        ...
        assert_eq!(
            player_health.0, 50,
            "Player should have 50 health after 5 attacks"
        );
        assert_eq!(
            enemy1_health.0, 50,
            "Enemy1 should have 50 health after 5 attacks"
        );
        ...
        assert!(
            client.get_entity_position(&enemy1_id).is_none(),
        ...
        assert_eq!(
            final_player_health.0, 50,
            "Player should still have 50 health"
        );
    }
    ...
    fn test_many_entities_stress_test() {
        ...
        assert_eq!(
            client.entity_count(),
            ENTITY_COUNT,
            "Should have spawned all entities"
        );
        ...
        for (index, &entity_id) in entity_ids.iter().enumerate() {
            if index % 2 == 0 {
                ...
                if let Some(health) = client.get_entity_health(&entity_id) {
                    assert_eq!(
                        health.0, 100,
                        "Odd-indexed entities should have full health"
                    );
                }
            }
        }
        ...
        assert_eq!(
            damaged_count,
            ENTITY_COUNT / 2,
            "Half the entities should be damaged"
        );
        assert_eq!(
            client.entity_count(),
            ENTITY_COUNT,
...
mod component_system_tests {
    ...
    fn test_gameposition_component_serialization() {
        ...
        assert_eq!(
            serialized.len(),
            8,
            "GamePosition should serialize to 8 bytes"
        );
    ...
    fn test_health_component_serialization() {
        ...
        assert_eq!(
            recovered_health.0, original_health.0,
            "Health value should match"
        );
    }
    ...
    fn test_invalid_deserialization() {
        ...
        assert!(
            invalid_pos.is_none(),
            "Invalid GamePosition data should not deserialize"
        );
        ...
        assert!(
            invalid_health.is_none(),
            "Invalid Health data should not deserialize"
        );
    }
    ...
    fn test_combat_system_direct() {
        ...
        assert_eq!(
            final_health.0, 0,
            "Health should saturate to 0, not underflow"
        );
