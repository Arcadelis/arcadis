...
import { WalletNetwork, type ISupportedWallet } from "@creit.tech/stellar-wallets-kit"
  ...
  useEffect(() => {
    const checkFreighter = () => {
      if (typeof window !== "undefined") {
        ...
        const hasFreighter = !!(window.freighterApi || (window as any).freighter || (window as any).stellar?.freighter)
          ...
          onClosed: (err) => {
            ...
            if (err) {
              if (err.message === "Modal closed") {
                resolve({ success: false, error: "User cancelled wallet connection" })
              } else {
  ...
  const signTransaction = async (xdr: string) => {
    try {
      ...
      if (freighterApi) {
        try {
          ...
          if (freighterApi.signTransaction) {
            ...
            signedTxXdr = await freighterApi.signTransaction(xdr, "Test SDF Network ; September 2015")
      ...
      try {
        ...
        throw new Error("Failed to sign transaction. Please ensure your wallet is connected and try again.")
      }
    } catch (error: unknown) {
